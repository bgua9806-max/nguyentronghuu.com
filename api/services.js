...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      const { data, error } = await supabase.from('services').select('*').order('created_at', { ascending: false }).limit(Number(limit));
      if (error) throw error;
    ...
    if (req.method === 'POST') {
      ...
      const servicePayload = {
        ...
        seo_description: seo_description || description || title,
      ...
      return res.status(201).json({
        ...
        url: `https://nguyentronghuu.com/services/${data.slug}`,
        data
      });
    }
    ...
    if (req.method === 'PUT' || req.method === 'PATCH') {
      ...
      let query = supabase.from('services').update(updateFields);
      ...
      if (error) throw error;
      return res.status(200).json({ success: true, message: 'Đã cập nhật dịch vụ!', data });
    }
