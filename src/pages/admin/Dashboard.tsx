...
import { FileText, Folder, Users, Activity, Eye, Clock, MessageSquare, Settings, ArrowUpRight, Sparkles, Loader2 } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setStatsData({
        ...
        newLeads: (contactsData || []).filter(contact => contact.status === 'new').length
      });
      
      setTopPosts(sortedPosts);
      setRecentContacts((contactsData || []).slice(0, 4));
  ...
  const stats = [
    { label: 'Tổng bài viết', value: statsData.totalPosts.toString(), icon: FileText, note: 'Nội dung' },
    { label: 'Tổng dự án', value: statsData.totalProjects.toString(), icon: Folder, note: 'Portfolio' },
    { label: 'Lượt xem bài viết', value: statsData.totalViews.toLocaleString(), icon: Activity, note: 'Tích lũy' },
    { label: 'Lead chờ xử lý', value: statsData.newLeads.toString(), icon: Users, note: 'CRM' },
  ];
        ...
        {stats.map((stat, idx) => (
          <div key={idx} className="group relative overflow-hidden rounded-sm bg-white p-6 shadow-[0_2px_20px_rgba(0,0,0,0.04)] transition-all hover:-translate-y-1 hover:shadow-[0_20px_40px_rgba(0,0,0,0.08)] border border-zinc-100">
            ...
            <div className="relative flex items-start justify-between">
              ...
              <div className="rounded-full bg-zinc-100 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-zinc-500">{stat.note}</div>
            </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="rounded-sm border border-zinc-100 bg-white p-6 sm:p-8 shadow-[0_2px_20px_rgba(0,0,0,0.04)] relative overflow-hidden">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
              <div>
                <h3 className="font-serif text-2xl text-zinc-950">Phân bổ lượt xem</h3>
                <p className="mt-1 text-sm text-zinc-500">Dữ liệu thực từ các bài viết nổi bật</p>
              </div>
              <span className="rounded-full bg-amber-50 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-amber-700">Dữ liệu tích lũy</span>
            </div>
            {isLoading ? (
              <div className="flex h-60 items-center justify-center"><Loader2 className="animate-spin text-amber-500" /></div>
            ) : topPosts.length === 0 ? (
              <div className="flex h-60 flex-col items-center justify-center rounded-sm border border-dashed border-zinc-200 bg-zinc-50/50 text-center">
                <Eye className="text-zinc-300" />
                <p className="mt-3 text-sm font-semibold text-zinc-700">Chưa có dữ liệu lượt xem</p>
                <p className="mt-1 text-xs text-zinc-400">Dữ liệu sẽ xuất hiện khi bài viết có lượt đọc.</p>
              </div>
            ) : (
              <div className="space-y-5">
                {topPosts.map((post, index) => {
                  const maxViews = Math.max(...topPosts.map(item => item.views || 0), 1);
                  const width = Math.max(6, ((post.views || 0) / maxViews) * 100);
                  return (
                    <div key={post.id}>
                      <div className="mb-2 flex items-center justify-between gap-4 text-xs">
                        <span className="truncate font-semibold text-zinc-700"><span className="mr-2 font-mono text-amber-600">0{index + 1}</span>{post.title}</span>
                        <span className="shrink-0 font-bold text-zinc-950">{(post.views || 0).toLocaleString()} lượt</span>
                      </div>
        ...
        <div className="space-y-8">
           <div className="rounded-sm bg-zinc-950 p-6 sm:p-8 text-white shadow-2xl shadow-zinc-900/20 relative overflow-hidden">
            ...
            <div className="space-y-3 relative z-10">
              ...
              <Link to="/admin/settings" className="w-full text-left group flex items-center gap-4 rounded-sm bg-white/5 p-4 transition-all hover:bg-white/10 border border-white/5 hover:border-white/10 active:scale-[0.98]">
          ...
          <div className="rounded-sm border border-zinc-100 bg-white p-6 sm:p-8 shadow-[0_2px_20px_rgba(0,0,0,0.04)]">
            ...
            <div className="relative space-y-6 before:absolute before:bottom-0 before:left-[19px] before:top-2 before:w-px before:bg-zinc-100">
              {recentContacts.length === 0 ? (
                <p className="relative z-10 rounded-sm bg-zinc-50 p-4 text-sm text-zinc-500">Chưa có yêu cầu liên hệ mới.</p>
              ) : recentContacts.map((contact) => (
                <div key={contact.id} className="relative flex gap-5">
                  ...
                  <div className="pt-1">
                    ...
                    <p className="mt-2 text-[10px] font-bold uppercase tracking-wider text-zinc-400">{new Date(contact.created_at).toLocaleDateString('vi-VN')}</p>
                  </div>
                </div>
              ))}
            </div>
            <Link to="/admin/contacts" className="mt-8 block w-full rounded-sm bg-zinc-50 py-3 text-center text-xs font-bold uppercase tracking-wider text-zinc-500 transition-colors hover:bg-zinc-100 hover:text-zinc-950 border border-zinc-200/50">Xem toàn bộ khách hàng</Link>
          </div>
        </div>
      </div>
    </div>
